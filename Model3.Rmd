...
---
# Classifying In_Person against Online (or Not In_Person)
...
#df_person <- df[!(df$pi_p2p_tree=="Digital"),]
#unique(df_person$pi_p2p_tree)
df_person<-df
#df_person <- filter(df, pi_p2p_tree == "Digital" | pi_p2p_tree == "Cards")
#unique(df_person$pi_p2p_tree)

dim(df_person)
head(df_person)

#df_person$amnt <- log(df_person$amnt)
#df_person$income_hh <- log(df_person$income_hh)
#df_person$other_debts <- log(df_person$other_debts)
df_person$pi_p2p_tree = as.factor(df_person$pi_p2p_tree)
df_person$work_cat = as.factor(df_person$work_cat)
df_person$merch = as.factor(df_person$merch)
df_person$race_cat = as.factor(df_person$race_cat)
df_person$race_cat <- relevel(df_person$race_cat, "1. White")
df_person$end_cash_bal_range= as.factor(df_person$end_cash_bal_range)

#df_person$merch_combined = as.factor(df_person$merch_combined)
#df_person$highest_education = as.factor(df_person$highest_education)
df_person$highest_education_cat = as.factor(df_person$highest_education_cat)
df_person$census_division = as.factor(df_person$census_division)
df_person$marital_status = as.factor(df_person$marital_status)
df_person$end_cash_bal_range = as.factor(df_person$end_cash_bal_range)
#df_person$tran = as.factor(df_person$tran)
#df_person$paypref_50to100 = as.factor(df_person$paypref_50to100)
#df_person$paypref_web = as.factor(df_person$paypref_web)


df_person$pi_p2p_tree = as.factor(df_person$pi_p2p_tree)
df_person$device = as.factor(df_person$device)
df_person$dc_logo_1 = as.factor(df_person$dc_logo_1)
#df_person$dc_rewards_1 = as.factor(df_person$dc_rewards_1)
#df_person$paypref_lt10 = as.factor(df_person$paypref_lt10)
df_person$gender = as.factor(df_person$gender)
df_person$shops_online = as.factor(df_person$shops_online)
#df_person$bill = as.factor(df_person$bill)
```

```{r}

#df_person<-na.omit(df_person)
#table(df_person$pi_p2p_tree)
#df_person$pi_p2p_tree <- ifelse(df_person$pi_p2p_tree == "Digital", 1 ,0)
table(df_person$in_person)
prop.table(table(df_person$in_person))

df_person<-na.omit(df_person)
table(df_person$in_person)
prop.table(table(df_person$in_person))

df_person <- ovun.sample(in_person~.,data = df_person, method = "both", p = 0.5)$data
#table(over)

#dim(over)


barplot(prop.table(table(df_person$in_person)),
        col = rainbow(2),
        ylim = c(0, 0.7),
...
```{r}
df_person<-fastDummies::dummy_cols(df_person,select_columns=c("work_cat","census_division","marital_status_cat","end_cash_bal_range","merch","race_cat","highest_education_cat","tran","paypref_50to100","paypref_web","device"),remove_selected_columns = T, remove_first_dummy = T)
...
df2018 <- filter(df_person, year == 2018)
df2019 <- filter(df_person, year == 2019)
df2020 <- filter(df_person, year == 2020)
...
```{r}
#colnames(train.df)
t1.logit.reg <- glm(in_person ~ ., data = train.df, family = binomial (link = "logit"))
options(scipen=999)
```
...
data.frame(actual = valid.df$in_person[1:5], predicted = t1.logit.reg.pred[1:5])

#class(t1.logit.reg.pred)

t1.logit.reg.pred2<-ifelse(t1.logit.reg.pred>0.45, 1 ,0)
confusionMatrix(as.factor(t1.logit.reg.pred2), as.factor(valid.df$in_person),positive='1')
```
```{r}
library(stats)
AIC(t1.logit.reg)
...
gain <- gains(valid.df$in_person, t1.logit.reg.pred, groups=10)
# plot lift chart
plot(c(0,gain$cume.pct.of.total*sum(valid.df$in_person))~c(0,gain$cume.obs),
xlab="# cases", ylab="Cumulative", main="", type="l")
lines(c(0,sum(valid.df$in_person))~c(0, dim(valid.df)[1]), lty=2)
```

```{r}
# compute deciles and plot decile-wise chart
heights <- gain$mean.resp/mean( valid.df$in_person)
...
classifier_RF = randomForest(x = train.df[-5],
                             y = train.df$in_person,
                             ntree = 10)
  
classifier_RF

y_pred = predict(classifier_RF, newdata = valid.df[-5])
  
# Confusion Matrix
confusion_mtx = table(valid.df[, 5], y_pred)
confusion_mtx
